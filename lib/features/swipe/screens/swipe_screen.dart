...
class _SwipeScreenState extends ConsumerState<SwipeScreen> {
  ...
  List<Recipe> _getFilteredRecipes(Set<String> pantry) {
    Iterable<Recipe> results = _recipes.where(
    ...
    if (_selectedMealTypes.isNotEmpty) {
      results = results.where((r) => _selectedMealTypes.contains(_norm(r.mealType)));
    }
    ...
    if (_selectedCuisines.isNotEmpty) {
      results = results.where((r) => _selectedCuisines.contains(_norm(r.cuisine)));
    }
    ...
    if (_selectedSkillLevels.isNotEmpty) {
      results = results.where((r) => _selectedSkillLevels.contains(_norm(r.skillLevel)));
    }
  ...
  void _onSwipeEnd(
    ...
    if (activity is Swipe) {
      ...
      final filteredRecipes = _getFilteredRecipes(_pantryKeySet(pantryItems));
  ...
  Future<void> _handleRightSwipe(Recipe recipe) async {
    ...
    try {
      ...
      if (!isPremium) {
        ...
        final needsReset = lastResetAt == null ||
        ...
        if (doNotShowAgain && !skipReminder) {
          await ref.read(appStateProvider.notifier).setSkipUnlockReminder(true);
        }
      }
      ...
      // Unlock failed (e.g., race/out-of-carrots); restore the card so swipe-right
      // remains synonymous with “unlock”.
      await undoLastSwipe();
    } catch (e) {
      await undoLastSwipe();
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredRecipes = _getFilteredRecipes(pantry);
                  ...
                  hScroll(
                    children: [
                      choice(
                        ...
                        onSelect: () => setState(() => _pantryFlex = 'exact'),
                      ),
                      choice(
      ...
      child: Column(
        ...
        children: [
          ...
          ...List.generate(recipe.ingredients.length, (index) {
            final rawKey =
                (index < ids.length) ? ids[index] : _norm(recipe.ingredients[index]);
  ...
  Future<void> _handleShowDirections(Recipe recipe) async {
    ...
    final isUnlocked =
        await ref.read(recipeServiceProvider).isRecipeSaved(userId, recipe.id);
    if (isUnlocked) {
      if (mounted) _openRecipeDetail(recipe);
      return;
    }
